...
static void get_time64(uint32_t t[2])
{
	 ...
	 * OK, so this isn't actually the time, but it's a 64-bit value unique for
	 * the current time, which is what we want.
	 */
...
static void btrseed(uint32_t seed)
{
	...
	/* Set key = key XOR time (i.e. add a bit more entropy) */
	get_time64(t);
