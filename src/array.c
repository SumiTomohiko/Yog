...
static void
copy_body(YogEnv* env, YogVal new, YogVal old, uint_t size, uint_t offset)
{
    uint_t i;
    for (i = 0; i < size; i++) {
        YogVal v = PTR_AS(YogValArray, old)->items[i];
        YogGC_UPDATE_PTR(env, PTR_AS(YogValArray, new), items[offset + i], v);
    }
}
...
#undef INCREASE_RATIO
        ...
        copy_body(env, new_body, old_body, PTR_AS(YogArray, array)->size, 0);
...
YogArray_extend(YogEnv* env, YogVal array, YogVal a)
{
    ...
    copy_body(env, to, from, YogArray_size(env, a), old_size);
